...
function fnConfirmMail(){
  ...
  indexlists=uniq(indexlists);//重複削除
  ...
  }//グルーピング　項目ごと(10時　など) 日時毎 ---- for B E
  ...
  for(var i=0;i<ssdatas.length;i++){//データの数だけ ---------- for A S
    ...
    if(lasttime < ssdatas[i][0]){//未送信のデータ---------if B S
      ...
      var datatitle = reTitle(datatitles,ssdatas[i]);  
      var content_datas = reData(datatitle,ssdatas[i],maxnum);
